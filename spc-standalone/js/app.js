...
const App = {
    ...
    renderCtlChart(samples, cfg) {
        ...
        this._renderSingleCtlChart(chartG1, ctl.g1, cfg);
        this._renderSingleCtlChart(chartG2, ctl.g2, cfg);
    },

    _renderSingleCtlChart(chart, g, cfg) {
        const oocIdx = SPCCalc.detectOOC(g.points, g.ucl, g.cl, g.lcl);
        const oocSet = new Set(oocIdx);
        ...
        chart.setOption({
            tooltip: {
                ...
                formatter: p => {
                    ...
                    const v = g.points[i];
                    return `#${i+1}: ${SPCCalc.round(v, cfg.digit)}${oocSet.has(i) ? ' ⚠ 异常' : ''}`;
                }
            },
            ...
            series: [{
                ...
                markLine: {
                    silent: true, symbol: 'none',
                    data: [
                        { yAxis: g.ucl, name: 'UCL', lineStyle: { color: '#e74c3c', width: 1 }, label: { formatter: `UCL=${SPCCalc.round(g.ucl, cfg.digit)}`, position: 'insideEndTop', fontSize: 10 } },
                        { yAxis: g.cl,  name: 'CL',  lineStyle: { color: '#27ae60', width: 1, type: 'dashed' }, label: { formatter: `CL=${SPCCalc.round(g.cl, cfg.digit)}`, position: 'insideEndTop', fontSize: 10 } },
                        { yAxis: g.lcl, name: 'LCL', lineStyle: { color: '#e74c3c', width: 1 }, label: { formatter: `LCL=${SPCCalc.round(g.lcl, cfg.digit)}`, position: 'insideEndBottom', fontSize: 10 } },
                    ]
                }
            }]
        }, true);
